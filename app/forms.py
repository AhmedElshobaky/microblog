...
class RegistrationForm(FlaskForm):
    ...
    validate_<field_name>, WTForms takes those as custom
    validators and invokes them in addition to the stock validators.
    '''
    def validate_username(self, username):
        user = User.query.filter_by(username=username.data).first()
        if user is not None:
            raise ValidationError('Please use a different usernama.')
...
class EditProfileForm(FlaskForm):
    ...
    # Overloaded constructor that accepts the original username 
    # as an argument.
    # 
